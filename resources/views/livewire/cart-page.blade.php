                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key='{{ $item['product_id'] }}'>
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            @if (isset($item['images']) && is_array($item['images']) && count($item['images']) > 0)
                                                ...
                                                @endphp
                                                <img class="h-16 w-16 mr-4" src="{{ $imageUrl }}"
                                                    alt="{{ $item['name'] }}">
                                            @else
                                                <img class="h-16 w-16 mr-4" src="/path/to/default/image.jpg"
                                                    alt="{{ $item['name'] }}">
                                            @endif
                                            <span class="font-semibold">{{ $item['name'] }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4">{{ Number::currency($item['unit_amount'], 'UZS') }}</td>
                                    <td class="py-4">
                                        <div class="flex items-center">
                                    ...
                                    <td class="py-4">{{ Number::currency($item['total_amount'],'UZS') }}</td>
            ...
            <div class="md:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span class="font-semibold">{{ Number::currency($grand_total,'UZS') }}</span>
                    </div>
                    @if ($cart_items)
                    <a href="/checkout" class="bg-blue-500 block text-center my-2 mt-4 text-white py-2 px-4 rounded-lg mt-5 w-full">Checkout</a>
                    @endif
                </div>
            </div>
        </div>
